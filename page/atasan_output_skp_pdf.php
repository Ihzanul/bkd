...
include ('../nigol.php');
	...
	$penilai = $_GET['penilai'];
        /*$hasil2  = mysqlii_query("select * from tbl_form_skp where penilai='$penilai'");
	  if (!$hasil2)
		die("Gagal Query data karena : ".mysqlii_error());

		
	if($row = mysqlii_fetch_array($hasil2))*/
	    $hasil  = mysqlii_query("select * from tbl_form_skp where tahun_skp=$year AND dinilai='".$nip."' ORDER BY tahun_skp");	
	
	$ambil = mysqlii_query("select nama_pns from tbl_pns where nip='".$nip."'");
	    if ($buaris = mysqlii_fetch_array($ambil))
   ...
   <tr> 
	...
	while($row = mysqlii_fetch_array($hasil))
	{
